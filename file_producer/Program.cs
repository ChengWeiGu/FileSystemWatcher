...
namespace ContinuousWriteFileApp
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var filePah = $@"D:\Side Work Data\Potwell_AOI\20210922\TestFileWatcher\{filename}";
        ...
        private static void AppendTextSimulator(string filePah)
        {
            ...
            for (var i = 0; i < 1000; i++)
            {
                ...
                {
                    string out_st = $@"{i},{date.ToShortDateString()},{rand.Next(1, 10)},{rand.Next(50, 100)},{(float)rand.Next(1, 10) / 10}";
                    Console.WriteLine(out_st);
                    sw.WriteLine(out_st);
                }
            }
        }
